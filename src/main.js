...
const { tweetCreation, tweetBid } = require('./twitter.js');
...
async function main() {
    ...
    options.toBlock = await getBlockHeight(); // Save current block height now so we don't miss any event

    logger.info(`Search from block ${CACHE.blockLast} to block ${options.toBlock} for auction logs!`);

    const creations = await contractAuction.getPastEvents('AuctionCreated', options);
    ...
    const bids = await contractAuction.getPastEvents('AuctionBid', options);
    if (bids != undefined) {
    ...
    const ends = await contractAuction.getPastEvents('AuctionEnded', options);
    if (ends != undefined) {
